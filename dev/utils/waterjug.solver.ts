...
export abstract class WaterJugSolver {
    ...
    public static FindSolution(input: WaterJugInput): WaterJugOutput | null {
        try {
            ...
            this.Solve(input.x, 0, input, 'x', xSteps);
            this.Solve(0, input.y, input, 'y', ySteps);
    ...
    private static Solve(x: number, y: number, input: WaterJugInput, mainJug: MainJug, steps: WaterJugOutput) {
        steps.push({ x, y }); //Include the present state in the steps array
    
        //In case the goal has been achieved, exit the function.
        if (x === input.z || y === input.z) {
            return;
        }
    
        //Prioritize filling up the main jug if it is designated as 'x'.
        if (mainJug === 'x') {
        ...
        } else { //Prioritize filling up the main jug (y) if possible.
            if (x === input.x) {
